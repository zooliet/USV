...
class DemoNode(Node):
    def __init__(self, interval=0.1, angular_speed=0.5, linear_speed=0.5, wps_file='wps.yaml', debug=False):
        super().__init__("test_nav_to_wps_node")
    ...
    def execute_callback(self, goal_handle):
        ...
        while rclpy.ok():  # or while True:
            ...
            if goal_handle.is_cancel_requested:
                ...
                self.get_logger().info("test_nav_to_wps action canceled")
                self.stop_drive()
        ...
        goal_handle.succeed()
        self.stop_drive()
        self.get_logger().info("test_nav_to_wps action completed successfully")
        result.success = True
        return result
...
def main(args=None):
    ...
    ap.add_argument(
        "-w", "--wps-file", default="wps.yaml", help="waypoints file"
    )
